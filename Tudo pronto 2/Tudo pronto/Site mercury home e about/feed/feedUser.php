...
if(isset($_POST['post-comment'])) {
    ...
    $curtidas = 0; // Inicialmente, definimos curtidas como 0
    date_default_timezone_set('America/Sao_Paulo');
    $data_postagem = date('Y-m-d H:i:s'); // Data e hora atual

    // Inserir os dados no banco de dados
    $query = "INSERT INTO feed (comentario, data_postagem, curtidas) VALUES ('$comentario', '$data_postagem', $curtidas)";
    $result = mysqli_query($conexao, $query);

    if (!$result) {
        echo "Erro ao inserir os dados no banco de dados: " . mysqli_error($conexao);
    }
}

// Processar a ação de curtir via AJAX
if (isset($_POST['like-comment'])) {
    $commentId = $_POST['comment-id'];
    $userId = $_SESSION['id']; // Supondo que você já tenha o ID do usuário na sessão

    // Verifique se o usuário já curtiu este comentário usando um cookie
    $cookieName = "liked_comment_$commentId";

    if (isset($_COOKIE[$cookieName])) {
        echo "<script>alert('Você já curtiu este comentário.');</script>";
    } else {
        // Atualize a tabela 'feed' para incrementar as curtidas
        $updateQuery = "UPDATE feed SET curtidas = curtidas + 1 WHERE id = $commentId";
        $updateResult = mysqli_query($conexao, $updateQuery);

        if ($updateResult) {
            // Defina um cookie para rastrear a curtida
            setcookie($cookieName, '1', time() + (86400 * 30), "/"); // O cookie expirará em 30 dias

            echo "<script>alert('Comentário curtido com sucesso.');</script>";
        }
    }
}

// Processar ação de edição via POST
...
<body>
        ...
        <div class="nav-list">
			...
			<li class="nav-item"><a href="#" class="nav-link">Events</a>
              <ul>
                ...
                <li class="updraft"><a href="../events/php/sarau/home.html" class="nav-link">Sarau</a></li>
                <li class="updraft"><a href="../events/php/sarau e paulo freire/Paulo Freire/pauloFreire.html" class="nav-link">Paulo Freire</a></li>
 ...
 <form action="feedUser.php" method="post">
    <div class="comment-feed">
    <p>Fique por dentro dos novos comunicados</p>
...
while ($row = mysqli_fetch_assoc($resultado)) {
    ...
    echo '<button type="submit" name="like-comment" class="curtir"><i class="far fa-heart"></i></button>';
              ...
              <footer>
                  ...
                  <p>Organização, comunicação e <br> união, na palma da sua mão. </p>
                      ...
                      <ul class="footer-list">
                          ...
                          <li>
                            <a href="../events/html/interclasse.html" class="footer-link">Interclasse</a>
                          </li>
                           <li>
                              <a href="../events/php/sarau/home.html" class="footer-link">Sarau</a>
                          </li>
                           <li>
                              <a href="../events/php/sarau e paulo freire/Paulo Freire/pauloFreire.html" class="footer-link">Paulo Freire</a>
                          </li>
